...
print(WordFeatures, type(WordFeatures))
...
objs = [clf,word_vectorizer,le]

joblib_file1 = "joblib_model.pkl"
joblib.dump(objs, joblib_file1)
...
# skill_list = list(skills_df.index)
resumeDataSet['skills'] = 0
...
for i in range(len(resumeDataSet)):
    for s in range(len(skill_list)):
        r = re.findall(r"" + re.escape(skill_list[s]) + r"",resumeDataSet['cleaned_resume'].iloc[i])
        if(r!=[]):
            r = list(set(r))
            resumeDataSet['skills'].iloc[i] = r 
...
a = resumeDataSet['skills'].iloc[1]
con = {d[1]:a}
# import json
# con.to_json("meta.json")
re = json.dumps(con)
print(re)
df = pd.DataFrame(con)
df.to_json("meta.json")
